...
n_avgs = 5e2 # SNR dependent, this is an upper bound
n_samples = 500 # no. of samples
anharm = 250 # MHz
qmm = QuantumMachinesManager(qm_ip)


def q_spectro(q_no, avgs, f_min, f_max, q_amp):
...
def run_spectro(q_no, avgs, f_min, f_max, q_amp):    

    qua_prog, f_LO, freqs = q_spectro(q_no, avgs, f_min, f_max, q_amp)
    ...
    return norm_sig, freq_array


def broad_sweep(q_no, avgs=n_avgs, f_min_MHz=-350, f_max_MHz=350, 
                q_amp=0.5, min_anharm_MHz=anharm-10):
    ...
    spectro_params = (q_no, avgs, f_min, f_max, q_amp)
    sig, freqs = run_spectro(*spectro_params) # sig is returned normalized
...
def fine_qubit_sweep(q_no, old_q_freq, old_q_fwhm, 
                     old_q_amp, avgs=n_avgs, into_amp=1/3):
...
def auto_spectro(q_no):
    ...
    prog_props, q_props, zero2by2_props, rerun_sweep = broad_sweep(q_no)
    print("------------------\n"
         f"Rerun is {rerun_sweep[0]}\n"
          "------------------")
    while rerun_sweep[0]:
        print("--------------------------------------------------------")
        ...
        prog_props, q_props, zero2by2_props, rerun_sweep  = broad_sweep(q_no, q_amp=temp_amp)
    ...
    # for i in range(1,4):
        q_freq, q_fwhm, q_amp = fine_qubit_sweep(q_no, old_q_freq, old_q_fwhm, old_amp)
        old_q_freq = q_freq
        old_q_fwhm = q_fwhm
        old_amp = q_amp
        q_fwhm = q_fwhm * u.GHz
