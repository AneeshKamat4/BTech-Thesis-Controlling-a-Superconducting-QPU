...
def initialize_Rabi(config, q_no, qmm, n_avg, t_min, t_max, dt, t_list_clk, wait_init, qe, rr, out, rabi_pi_amp, show_Rabi=False):
    ...
    pi_len_ns = config["pulses"]["const_pulse"]["length"] 
    init_pi_amp = rabi_pi_amp*rabi_pi_len/pi_len_ns
	...
	# print(f'The initial amplitudes for X_pi, Y_pi, X_piby2, Y_piby2 are: {initial_amps}')
	return initial_amps
...
def opt_allxy(opt_amps, config, q_no, qmm, n_avg, t_min, t_max, dt, t_list_clk, wait_init, qe, rr, out, rabi_pi_amp, show_Rabi=False, print_Rabi=False):
